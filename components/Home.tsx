    ...
    <div className="max-w-5xl mx-auto mb-20 px-4 relative z-30 reveal">
      <div className="bg-white/95 backdrop-blur-md rounded-[2rem] md:rounded-[2.5rem] shadow-[0_20px_50px_rgba(0,0,0,0.08)] border border-sky-100 p-6 md:p-12 overflow-hidden relative">
        ...
        <div className="grid lg:grid-cols-2 gap-8 md:gap-12 items-center text-right">
          <div>
            <div className="inline-block bg-sky-100 text-sky-700 text-[10px] font-black px-3 py-1 rounded-full mb-4 uppercase tracking-widest">
              כלי אינטראקטיבי
            </div>
            <h2 className="text-2xl md:text-3xl font-black text-sky-950 mb-4 md:mb-6">מחשבון המחיר האמיתי</h2>
          ...
          <div className="grid grid-cols-2 gap-3 md:gap-5">
            ...
            <div className="bg-sky-600 p-4 md:p-8 rounded-2xl md:rounded-3xl text-center shadow-xl">
              <div className="text-sky-100 text-[8px] md:text-[10px] mb-2 uppercase font-black tracking-widest">חיסכון שנתי</div>
            ...
            <div className="bg-sky-950 p-6 md:p-8 rounded-2xl md:rounded-3xl text-center text-white col-span-2 shadow-2xl">
              <div className="text-yellow-400 text-[8px] md:text-[10px] mb-2 uppercase font-black tracking-widest">בעוד עשור תוכלו לחסוך</div>
              <div className="text-3xl md:text-5xl font-black text-white mb-2 md:mb-3">₪{stats.tenYears}</div>
              <div className="text-xs md:text-sm text-sky-200/50 font-medium tracking-wide">זמן להשקיע בעצמכם ובמשפחה</div>
            </div>
            <div className="bg-yellow-50 p-4 md:p-6 rounded-2xl border border-yellow-100 text-center col-span-2 flex items-center justify-center gap-3 md:gap-5">
              <div className="bg-sky-500 p-2 md:p-3 rounded-full text-white shadow-lg">
                <svg className="w-4 h-4 md:w-6 md:h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" /></svg>
              </div>
    ...
    <div className="overflow-x-hidden">
      ...
      <section className="relative hero-gradient text-white pt-16 md:pt-24 pb-32 md:pb-44 px-4 overflow-hidden min-h-[70vh] md:min-h-[85vh] flex items-center">
        ...
        <div className="max-w-6xl mx-auto text-center relative z-10 animate-fade-in-up px-2">
          ...
          <div className="flex flex-col sm:flex-row justify-center gap-4 px-4">
            ...
            <button 
              onClick={() => onNavigate('articles')}
              className="bg-white/10 backdrop-blur-md hover:bg-white/20 text-white border border-white/30 px-8 md:px-12 py-4 md:py-5 rounded-xl md:rounded-2xl font-black text-lg md:text-xl transition-all active:scale-95"
            >
              לספריית המחקרים
            </button>
          </div>
      ...
      <section className="py-16 md:py-24 bg-white">
        <div className="max-w-7xl mx-auto px-4 grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-10">
          <div className="text-center p-6 md:p-10 rounded-[2rem] md:rounded-[2.5rem] bg-sky-50/50 border border-sky-50 hover:bg-white hover:shadow-2xl transition-all cursor-pointer reveal" onClick={() => onNavigate('articles')}>
          ...
          <div className="text-center p-6 md:p-10 rounded-[2rem] md:rounded-[2.5rem] bg-yellow-50/50 border border-yellow-50 hover:bg-white hover:shadow-2xl transition-all cursor-pointer reveal" onClick={() => onNavigate('forum')}>
          ...
          <div className="text-center p-6 md:p-10 rounded-[2rem] md:rounded-[2.5rem] bg-sky-50/50 border border-sky-50 hover:bg-white hover:shadow-2xl transition-all cursor-pointer reveal" onClick={() => onNavigate('about')}>
            <div className="w-16 h-16 md:w-20 md:h-20 bg-sky-100 text-sky-600 rounded-2xl md:rounded-3xl flex items-center justify-center mx-auto mb-6 md:mb-8">
      ...
      <section className="py-16 md:py-24 bg-sky-50/30 reveal">
        <div className="max-w-7xl mx-auto px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8">
            {featuredResearch.map((item, idx) => (
              <div key={item.id} onClick={() => onNavigate('articles')} className="bg-white p-6 md:p-10 rounded-[2rem] shadow-sm border border-sky-50 hover:shadow-2xl transition-all cursor-pointer reveal">
