  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-12">
      <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-6 animate-fade-in-up">
        <div>
          <h1 className="text-4xl font-black text-slate-800 mb-3">פורום קהילה וידע</h1>
          <p className="text-slate-500 text-lg font-medium">מרחב בטוח לשיתוף, התייעצות ותמיכה הדדית</p>
        </div>
        <button 
          onClick={handleNewPost}
          className="bg-emerald-600 text-white px-8 py-3 rounded-2xl font-black hover:bg-emerald-700 hover:shadow-xl hover:-translate-y-1 active:scale-95 flex items-center gap-3 transition-all"
        >
      ...
      <div className="space-y-8">
        {posts.map((post, idx) => (
          <div 
            key={post.id} 
            className="bg-white p-8 rounded-[2rem] shadow-sm border border-slate-100 hover:border-emerald-200 hover:shadow-xl transition-all animate-fade-in-up"
            style={{ animationDelay: `${(idx + 1) * 150}ms` }}
          >
            <div className="flex items-center gap-4 mb-6">
              <div className="w-12 h-12 bg-emerald-100 text-emerald-700 rounded-2xl flex items-center justify-center font-black text-xl shadow-inner group-hover:rotate-3 transition-transform">
                {post.author[0]}
              </div>
              <div>
                <div className="font-black text-slate-800 text-lg">{post.author}</div>
            ...
            <div className="flex items-center gap-8 pt-6 border-t border-slate-50">
              <button 
                ...
                className={`flex items-center gap-2.5 text-sm font-black transition-all transform active:scale-125 ${post.isLiked ? 'text-emerald-600' : 'text-slate-400 hover:text-emerald-600'}`}
              >
      ...
      <div className="mt-20 bg-emerald-50 p-10 rounded-[3rem] border border-emerald-100 flex flex-col md:flex-row items-center gap-10 animate-fade-in delay-500 shadow-inner">
        <div className="flex-grow">
          ...
          <ul className="text-emerald-800 space-y-3 text-base font-medium list-none">
            <li className="flex items-center gap-2"><span className="w-1.5 h-1.5 bg-emerald-500 rounded-full"></span>שמירה על שיח מכבד וענייני</li>
            <li className="flex items-center gap-2"><span className="w-1.5 h-1.5 bg-emerald-500 rounded-full"></span>חל איסור מוחלט על פרסום מסחרי</li>
            <li className="flex items-center gap-2"><span className="w-1.5 h-1.5 bg-emerald-500 rounded-full"></span>אין לראות בתכנים ייעוץ רפואי מחייב</li>
            <li className="flex items-center gap-2"><span className="w-1.5 h-1.5 bg-emerald-500 rounded-full"></span>אנחנו כאן כדי לתמוך, לא לשפוט</li>
          </ul>
        </div>
        <div className="relative group">
          <div className="absolute -inset-1 bg-emerald-200 rounded-full blur opacity-50 group-hover:opacity-100 transition duration-1000"></div>
